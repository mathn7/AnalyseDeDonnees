...
@testset "Tests TP3" begin
    ...
    rm(cheminTP3*"resultats-ex1.mat",force = true)
    rm(cheminTP3*"resultats-ex2.mat",force = true)

    fleurs = ["pensees","oeillets","chrysanthemes"]
    donnees   = matread(cheminTP3*"donnees.mat")
    tol_erreur = 1e-5
    ...
    @testset "Tests exo 3" begin 
        @test 76.67 ≈ tp3_exercice3(false,cheminTP3) atol = 1                 # test accuracy == 76.67 %   
        @test 83.33 ≈ tp3_exercice3bis(false,cheminTP3,[.5, .3, .2]) atol = 1 # test accuracy == 83.33 %   
